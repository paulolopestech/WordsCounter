...
const createClientInstance = () => new CountEvenCharactersWordsClient('Cliente');
...
describe('Testing count method', () => {
    ...
    test('Should return 0 if there are no even number of characters', () => {
        const client = createClientInstance();
        const numberOfWords = client.count('A, B, C');
        expect(numberOfWords).toBe(0);
    });

    test('Should return the number of words with even number of characters', () => {
        const client = createClientInstance();
        const numberOfWords = client.count('AB, B, C');
        expect(numberOfWords).toBe(1);
    });
})